...
namespace Setchin.NethardCloudMusic
{
    public class Song : IEquatable<Song>
    {
        ...
        {
            public SongUrlDto dto;

            public bool IsChargeable(ApiOperator @operator, long id)
            {
                if (dto == null)
                {
                    string content = @operator.Get("song/url", new { Id = id, Br = 128000 });
                    dto = JsonConvert.DeserializeObject<SongUrlDto>(content);
                }

                return dto.Datas[0].IsChargeable;
            }

            public string Url(ApiOperator @operator, long id)
            {
                if (dto == null)
                {
                    string content = @operator.Get("song/url", new { Id = id, Br = 128000 });
                ...
                return dto.Datas[0].Url;
            }

            public class SongUrlDto
            {
                [JsonProperty("data")]
                public DataDto[] Datas;
                ...
                public void Download(ApiOperator @operator, string url)
                { 
